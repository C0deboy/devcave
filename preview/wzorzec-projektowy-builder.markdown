...
Używając podejścia z bezargumentowym konstruktorem i setterami poprawia czytelność kodu:
...
Jednak takie podejście nie gwarantuje nam spójności klasy. Nie ma tutaj opcji, aby zmusić klienta, żeby ustawił wszystkie wymagane pola. Uniemożliwia to też stworzenie klasy niezmiennej, co wymaga dodatkowego wysiłku, aby zapewnić *thread safety*. I tu z pomocą przychodzi *Builder*. 

# Budowniczy (Builder) - wzorzec projektowy

Ten wzorzec projektowy ma kilka wariantów. W książce *Effective Java* poruszany jest tylko ten pierwszy. Dwa pozostałe są jako dodatek, który warto porównać.
...
```
W metodzie {% code java %}public Goal build(){% endcode %} możemy zweryfikować przed zbudowaniem obiektu czy wszystkie wymagane pola zostały zainicjowane (istnieje też inny fajny sposób, aby wymusić ustawienie wszystkich pól w builderze, napiszę o tym osobny post - będzie dostępny w najbliższą środę), a w poszczególnych *setterach* możemy przeprowadzić walidację pól. Stosując wzorzec *builder* możemy zagwarantować, że stworzona zostanie poprawna i kompletna klasa, zachowując czysty i łatwy w użyciu kod. Instancjowanie klasy wygląda wtedy tak:
...
Goal goal = new Goal.Builder()
    ...
    .build();
```
Jak widać jest to dużo łatwiejsze do czytania jak i używania. Kolejne pola możemy podać w dowolnej kolejności, jest to tak zwane *fluent API*. 
...
Dzięki temu możemy zapewnić, że klasa będzie zawsze spójna.
...
Dodatkowym plusem jest to, że możemy dodawać elementy np. do list w dynamiczny sposób, zamiast wypełniać całą listę wcześniej i przekazywać ją do konstruktora. W naszym przykładzie *buildera* będzie to wyglądać tak:
...
Goal goal = Goal.builder()
    ...
    .build();
```
Podobnie można by zrobić z *checklistą*.


## Fluent Builder

Tu jedyna różnica polega na tym, że *Builder* implementujemy jako osobną klasę:
...
public final class GoalBuilder {
  ...
  public GoalBuilder levels(List<Level> levels) {
    this.levels = levels;
    return this;
  }
...
Pisanie builderów jest nużące i powtarzalne, dlatego powstały liczne pluginy do IDE, które generują je za nas. Dla InteliJ mamy na przykład plugin *Builder Generator*. Po zainstalowaniu wystarczy kliknąć <kbd>Alt</kbd>+<kbd>Shift</kbd>+<kbd>B</kbd> i wyskoczy nam opcja do wygenerowania buildera. <br> <br>
    Powstały również do tego całe biblioteki. Jedną z nich jest np. [Lombok](https://projectlombok.org/) (Który udostępnia szereg innych funkcji). Wystarczy dorzucić do naszej klasy adnotację {% code java %}@Builder{% endcode %} i już możemy korzystać z *Buildera* za pomocą statycznej metody {% code java %}builder(){% endcode %}.
...
Ten wzorzec nie ma zastosowania w naszym przykładzie z tworzeniem klasy *Goal*, ponieważ "cel" nie ma zazwyczaj predefiniowanych konkretnych implementacji. Moglibyśmy tak zrobić, ale to raczej mało realny przykład. Jako przykład mogę podać narzędzie do zbierania statystyk dla języków programowania, które tworzyłem dla mojej strony [jaki-jezyk-programowania.pl](https://jaki-jezyk-programowania.pl/). Tam zaprojektowałem kod w podobny sposób.
...
```
U mnie jest to scraper, ale zasada podobna. Po prostu zamiast metod buildName() i buildData() są getName() i getData(). Buildery mają zazwyczaj więcej takich metod, ponieważ stosuje je się do tworzenia skoplikowanych obiektów.
